---
sidebar_position: 6
hide_title: true
---









##  Références et ressources

1.  Nations Unies. (2025). Principes et recommandations concernant les recensements de la population et des logements. Révision 4. Accessible à l'adresse : https://unstats.un.org/unsd/publication/SeriesM/Series_M67Rev4en.pdf

2.  Division de statistique des Nations Unies. (2010). Directives opérationnelles relatives aux enquêtes post-recensement. Accessible à l'adresse : https://unstats.un.org/unsd/demographic/standmeth/handbooks/Manual_PESen.pdf

3.  Office national des statistiques du Royaume-Uni. GitHub - ONSdigital/pes_match : Ensemble de fonctions permettant de nettoyer et de faire correspondre les données du recensement avec celles de l'enquête post-recensement. Disponible à l'adresse : https://github.com/ONSdigital/pes_match/tree/main

4.  Ministère de la justice du Royaume-Uni. (2021). Splink : Bibliothèque libre du ministère britannique de la Justice relative à l'appariement probabiliste d'enregistrements à grande échelle. Accessible à l'adresse : https://www.gov.uk/government/publications/joined-up-data-in-government-the-future-of-data-linking-methods/splink-mojs-open-source-library-for-probabilistic-record-linkage-at-scale

5.  Office national des statistiques du Royaume-Uni. Système d'appariement administratif CROW. Disponible à l'adresse : https://github.com/ONSdigital/ons-crow

6.  Peter Benton. (2010). Trout, Catfish and Roach, guide du débutant relatif aux estimations démographiques issues du recensement. Accessible à l'adresse : https://docslib.org/doc/2577669/trout-catfish-and-roach-the-beginner-s-guide-to-census-population-estimates

7.  Institut national de la statistique du Rwanda. Rapport thématique RPHC5 : Enquête post-recensement. Accessible à l'adresse : https://www.statistics.gov.rw/data-sources/censuses/Population-and-Housing-Census/fifth-population-and-housing-census-2022/rphc5-thematic-reports/rphc5-thematic-report-post-enumeration-survey


